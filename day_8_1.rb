...
INPUT.split("\n").each do |row|
  inst = row.split(" ")
  registers[inst[0]] = 0 unless registers.keys.include?(inst[0])
  registers[inst[4]] = 0 unless registers.keys.include?(inst[4])
  inst[1] = "+=" if inst[1] == "inc"
  inst[1] = "-=" if inst[1] == "dec"
  if eval(registers[inst[4]].to_s+inst[5]+inst[6])
    eval("registers["+'"'+inst[0]+'"'+"]"+inst[1]+inst[2]) 
  end
end
