  ...
  until towers.count == 1
    towers.each do |k, v|
      if v[1] != []
        ...
        if v[1].count == compare_list.count
          compare_weight = 0
          diff = false
          compare_list.each_with_index do |title,idx|
            compare_weight = towers[title][0] if idx == 0
            diff = true if compare_weight != towers[title][0] 
          end
          if diff
            title_box = []
            weight_box = []
            compare_list.each_with_index do |title,idx|
              unless title_box.count >= 4
                if towers[title][0] != towers[compare_list[idx+1]][0]
                  title_box.push(title,compare_list[idx+1]) 
                  weight_box = [towers[title][0], towers[compare_list[idx+1]][0]]
                end
                title_box.push(title,compare_list[(idx+2)%compare_list.size]) if towers[title][0] != towers[compare_list[(idx+2)%compare_list.size]][0]
              end
            end
            diff_weight = 0
            diff_title = title_box.group_by{|i| i}.reject{|k,v| v.one?}.keys[0]
            if weight_box.max == towers[diff_title][0]
              diff_weight = weight_box.max - weight_box.min
            else
              diff_weight = weight_box.min - weight_box.max
            end
            return [diff_title, diff_weight]
          end
          new_v = []
          v[1].each do |title|
            if towers[title][1] == []
          ...
          towers[k] = [towers[k][0] + compare_list.count * compare_weight, new_v]
        end
      end
    end
  end
...
array.each do |row|
  ...
  if row.include?("->")
    disc = row[/->.*/]
    disc.slice!(0,3)
    towers[title] = [weight, disc.split(", ")]
  else
    towers[title] = [weight,[]]
  end
end
